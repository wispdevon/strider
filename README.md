...
npm run build:cloudflare    # Build the OpenNext Cloudflare bundle
npm run preview             # Build and preview with the Cloudflare Workers runtime
npm run deploy              # Build and deploy with the Cloudflare OpenNext adapter
...
and `npm run deploy`. The current app still uses native
